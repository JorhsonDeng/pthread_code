...
int main(int argc, char *argv[])
{
    ...
    int epfd;       //epoll file describe
    epoll_event events[1000];
    epfd = epoll_create(5);     //返回一个描述符，后面要记得close，size = 5
    assert(epfd != -1);
    ...
    while(true)
    {
        ...
        for(int i=0; i<number; i++)
        {
            ...
            printf("\r\n number = %d, event = %d, sockfd = %d ",number,events[i].events,sockfd);
            if(sockfd == listenfd)//有新用户连接
            {
               ...
               /* if(http_coon::m_user_count > MAX_FD)
                ...
                //初始化客户连接
                cout << "epfd:"<<epfd << " client_fd:" << client_fd << endl;
                addfd(epfd, client_fd, true);
                cout << "client_fd:" << client_fd << "****\n";
                //client_fd是新的文件描述符，把epfd和client_fd加入到这个对象中的变量里
            ...
            {
                cout<<"receive: client sockfd is: "<<sockfd<<endl;
            ...
            {
                cout<<"send: client sockfd is: "<<sockfd<<endl;
