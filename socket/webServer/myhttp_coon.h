...
public:
    ...
    FILE_REQUESTION代表GET方法资源请求；I
    NTERNAL_ERROR代表服务器自身问题；NOT_FOUND代表请求的资源文件不存在；
...
{
    //首先向epoll中把client_fd事件删除，其次把client_fd这个socket描述符关闭
...
{
    //must be set to zero before to write the memory
...
bool http_coon::bad_respond()//400
{
    ...
    sprintf(filename,"/mnt/hgfs/Host_Ubuntu/socket/webServer/%s",url);
    struct stat my_file;
...
void http_coon::dynamic(char *filename, char *argv)
{
    ...
    {
        cout << "\t\t\t\tError requestion!!!\n\n";
        sum = number[0]*number[1];
        sprintf(body,"<html><body>\r\n<p>Error requestion, please check the information again!!! </p><hr>\r\n</body></html>\r\n");
        sprintf(requst_head_buf,"HTTP/1.1 200 ok\r\nConnection: close\r\ncontent-length: %d\r\n\r\n",strlen(body));
    }
}
...
http_coon::HTTP_CODE http_coon::head_analyse(char *temp)
{
    if(temp[0]=='\0')
    {
        cout<<"we get complete request information!!!"<<endl;
        //获得一个完整http请求
...
{
    char path[100]="/mnt/hgfs/Host_Ubuntu/socket/webServer";
    char* ch;
    if(ch=strchr(url,'?'))
    {
        argv = ch+1;
        *ch = '\0';
        strcpy(filename,url);
        cout<<"ch filename is:"<<filename<<endl;
        return DYNAMIC_FILE;
    }
    else{
            ...
            {
                cout<<"do_file:deal with BAD_REQUESTION"<<endl;
